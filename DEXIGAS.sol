...
interface UTL {
    ...
    /**
     * @dev Emitted when `value` tokens are moved from platform account to
     * another (`to`).
     *
     * Note that `value` may be zero.
     */
...
contract DEXIGAS is Context, IBEP20, Ownable, UTL {
    ...
    // The blacklist function prevents hackers that stole tokens from interacting with the stoken tokens. 
    //This prevents the hackers from profiting of their criminal actions.
    ...
    constructor() public {
        ...
        _symbol = "DXG";
        _decimals = 18;
        _totalSupply = 250000 * 10**6 * 10**18;
    ...
    /**
     * @dev Throws if called by any account other than platform account.
     */
    modifier isPlatform() {
    ...
    /**
     * @dev Throws if called by any account other than safe accounts.
     */
    modifier isSafeAddress() {
        require(owner() == _msgSender(), "Ownable: caller is not a safe account");
        _;
    }
